...
% Add random perturbations to the source locations:
receiver_locations = source_locations + (rand(size(source_locations))-.5)*2 * 50E-6;
...
w = 5;
tic;
img = SLSC_1rec_free_position_dll(rf_data,source_locations,receiver_locations,image_coordinates,c,fsamp,m,w,Nx,Ny,Nz);
img = reshape(img,Nx,Ny,Nz);
img = 20*log10(squeeze(img)');
imgSLSC = img - max(img(:));
tSLSC = toc;
